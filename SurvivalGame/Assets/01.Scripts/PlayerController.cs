...
public class PlayerController : MonoBehaviour
{
    ...
    {//스페이스바를 한 번 눌렀을 경우 / 땅위에 있을 경우에 / GetCurrentSP가 0보다 클때
    ...
    {//shitf키를 누르면 달릴 수 있도록...
        if (Input.GetKey(KeyCode.LeftShift) && theStatusController.GetCurrentSP() > 0) //LeftShift 를 누르게 되면
        {
            Running();
        }
        if (Input.GetKeyUp(KeyCode.LeftShift) || theStatusController.GetCurrentSP() <= 0) //LeftShift에서 손을 떼면
        {
            RunningCancel();
        }
    }
